...
syn keyword cfScope contained cfcatch variable var variables application server session client form url attributes
syn keyword cfScope contained arguments
syn keyword cfBool contained yes no true false

" Operator strings.
syn keyword cfOperator contained And All
...
"lasso
syn match cfOperator "[-=+%^&|*!.~?:]" contained display
syn match cfOperator "[-+*/%^&|.]="  contained display
syn match cfOperator "/[^*/]"me=e-1  contained display
syn match cfOperator "\$"  contained display
syn match cfOperator "&&\|\<and\>" contained display
syn match cfOperator "||\|\<x\=or\>" contained display
syn match cfRelation "[!=<>]=" contained display
syn match cfRelation "[<>]"  contained display
syn match cfMemberSelector "->"  contained display
syn match cfVarSelector  "\$"  contained display
syn match cfOperator "!>>" contained display
syn match cfOperator "!==" contained display
syn match cfOperator "@" contained display
syn match cfOperator "\\\\" contained display
...
" Tag names.
"lasso
syn keyword cfTagName contained Var Variable
syn keyword cfTagName contained Action_Param
syn keyword cfTagName contained Insert
syn keyword cfTagName contained Field
"
...
" Tag parameters.
" lasso
syn keyword cfArg Value
syn keyword cfArg Database
syn keyword cfArg Insert
